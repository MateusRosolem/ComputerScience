...
void aula() {
  ...
  Taluno aluno[50];
  char nomedisc[20], busca[20], filename[20], filename2[20], busca3[20], busca5,
      testacabou[20];
...
void d() {
  ...
  do {
    ...
    while (strcmp("acabou", aluno.nome_aluno) != 0) {
    fscanf(alunos, "%s ", aluno.nome_aluno);
        if (strcmp("acabou", aluno.nome_aluno) == 0) break;
...
void e() {
int exit=1, n_aulas=0, indicedisc;
char filename[20], busca[20], nomedisc[20], opt;
FILE *fpdisciplina, *materias, *alunos, *aulas, *fprel;
disciplina disc;
Taluno aluno;
Taula aula;
Tavaliacao prova;
  ...
  materias = fopen("disciplinas.txt", "r");
  while (exit) {
    fscanf(materias, "%s %d ", busca, &indicedisc);
    if (strcmp(busca, nomedisc) == 0) {
      exit = 0;
    }
  } 
